...
import cn.jfinalbbs.user.WebUser;
    ...
    public void qqlogincallback() throws QQConnectException {
        ...
        if (accessTokenObj.getAccessToken().equals("")) {
            ...
            if (user == null) {
                ...
                user.set("expire_time", expire_in).set("open_id", openID).update();
            }
...
//    public void weblogin() {
//        redirect("http://www.cxzc.com:8080/rest/bbslogin?retUrl=http://bbs.cxzc.com:8088/websitecallback");
//    }

    /**
     * 业务站登录
     */
    public void websitecallback() throws cn.weibo.org.json.JSONException, JSONException {

            if(!StringUtils.isEmptyOrWhitespaceOnly(getPara("id"))){
                String gender = "未知";
                if (getPara("sex").equals("M")) {
                    gender = "男";
                } else if (getPara("sex").equals("F")) {
                    gender = "女";
                }
                Date expire_in = DateUtil.getDateAfter(new Date(), 10000);
                User user = User.me.findByOpenID(getPara("id"), "website");
                if (user == null) {
                    ...
                    user.set("id", StrUtil.getUUID())
                            ...
                            .set("score", 0)
                            .set("gender", gender)
                            .set("avatar", getPara("photo"))
                            .set("open_id", getPara("id"))
                            .set("expire_time", expire_in)
                            .set("in_time", new Date())
                            .set("mission", new Date())
                            .set("thirdlogin_type", "website").save();
                }
                setSessionAttr(Constants.USER_SESSION, user);
                setCookie(Constants.USER_COOKIE, StrUtil.getEncryptionToken(user.getStr("token")), 30 * 24 * 60 * 60);
                redirect(Constants.getBaseUrl() + "/");
